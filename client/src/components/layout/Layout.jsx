...
export function Layout({ children }) {
  ...
  const isDesigner = pathname.startsWith('/designer') || pathname.startsWith('/live') || pathname.startsWith('/dispatch');
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header className="top-0 z-40 border-b-2 border-[#854d0e] bg-(--surface)/90">
        ...
        >
          ...
          <div className="flex shrink-0 items-center gap-4">
            <Link to="/designer" className="flex items-center gap-2 truncate text-sm font-semibold tracking-tight text-(--page-fg) transition-colors hover:text-(--accent) sm:text-base">
              <Icon icon="material-symbols:grid-on-rounded" className="h-5 w-5 shrink-0" aria-hidden="true" />
              <span className="truncate">Designer</span>
            </Link>
            <Link to="/live" className="flex items-center gap-2 truncate text-sm font-semibold tracking-tight text-(--page-fg) transition-colors hover:text-(--accent) sm:text-base">
              <Icon icon="material-symbols:map" className="h-5 w-5 shrink-0" aria-hidden="true" />
              <span className="truncate">Live</span>
            </Link>
            <Link to="/dispatch" className="flex items-center gap-2 truncate text-sm font-semibold tracking-tight text-(--page-fg) transition-colors hover:text-(--accent) sm:text-base">
              <Icon icon="material-symbols:route" className="h-5 w-5 shrink-0" aria-hidden="true" />
      ...
      {!isDesigner && <EmergencyStopButton />}
      {!isDesigner && <FloatingNavigation />}
      <main className={`flex flex-1 flex-col ${isDesigner ? '' : 'pb-4'}`}>{children}</main>
      {!isDesigner && (
